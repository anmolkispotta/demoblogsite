...
?>
<div class="coneblog-author-box layout-<?php echo esc_attr($layout) ?><?php echo ( esc_attr($settings['default_shadow'] == 'yes' ? ' has-shadow' : '') ) ?>">
    <div class="cb-authorbox-left-col">
        <?php if('yes' === $settings['show_avatar']): ?>
        <div class="cb-avatar<?php echo ( esc_attr($settings['show_avatar_border'] == 'yes' ? ' has-border' : '') ) ?>">
            <?php
            if($avatar) {
                ...
                <?php
                    if ( $settings['social_links_list'] ) {
                        foreach (  $settings['social_links_list'] as $item ) {
                            $target = $item['social_link']['is_external'] ? '_blank' : '_self';
                            $nofollow = $item['social_link']['nofollow'] ? 'nofollow' : 'follow';
                            ?>
                            <li>
    ...
    <div class="cb-authorbox-right-col">
        ...
        <div class="cb-author-title">
            <?php
                $target_author = $settings['author_business_url']['is_external'] ? '_blank' : '';
                $nofollow_author = $settings['author_business_url']['nofollow'] ? 'nofollow' : '';
            ?>
