...
class Helper
{
     ...
     * @return string
     */
    public static function coneblog_post_format_icon() {
    ...
    public static function coneblog_post_term_box() {
        ...
        $categories = get_the_category();
        if ( ! empty( $categories ) ) {
            echo '<a href="' . esc_url( get_category_link( $categories[0]->term_id ) ) . '" class="grid-post-term"><span class="term-icon"><i class="'.$format_icon.'"></i></span><span class="term-name">' . esc_html( $categories[0]->name ) . '</span></a>';
        }
    }
    /**
     * Get All Post Types
